  ...
  try {
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        const isGeminiModel = selectedChatModel.startsWith("google/");
        const supportsThinking = selectedChatModel === "google/gemini-3-pro-preview";

        // Get MCP tool names for active tools list
        const mcpToolNames = Object.keys(mcpContext.tools);

        // All tools come from MCP servers now
        const allTools = {
          ...mcpContext.tools,
        };

        // Build active tools list from MCP tools only
        const allActiveTools = isReasoningModel ? [] : mcpToolNames;
      ...
      onError: () => {
        ...
        disconnectMCPClients(mcpContext);
