...
import { Loader2, X, ExternalLink, ChevronDown, ChevronUp, BookOpen, Pencil, Coins } from "lucide-react";
import type { UserContract, EthereumNetwork } from "@/lib/db/schema";
import { getBlockExplorerUrl, getMneeAddress } from "@/lib/contracts/network-config";
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50">
      <div className="fixed inset-4 bg-background border rounded-lg shadow-lg flex flex-col">
        ...
        <Tabs defaultValue="mnee" className="flex-1 flex flex-col overflow-hidden">
          <TabsList className="mx-4 mt-4">
            <TabsTrigger value="mnee" className="flex items-center gap-2">
          ...
          <TabsContent value="mnee" className="flex-1 overflow-auto p-4">
            <MneeTokenPanel
...
// MNEE Token Panel for approve/allowance operations
function MneeTokenPanel({ spenderAddress, network }: { spenderAddress: string; network: EthereumNetwork }) {
  ...
  const checkAllowanceAndBalance = async () => {
    ...
    try {
      ...
      if (currentNetwork !== network) {
        ...
        if (!switched) {
          throw new Error(`Please switch MetaMask to ${network === "mainnet" ? "Ethereum Mainnet" : "Sepolia"}`);
        }
      }
      ...
      const [allowanceResult, balanceResult] = await Promise.all([
        readContract(mneeAddress, ERC20_ABI, "allowance", [address, spenderAddress], network),
        readContract(mneeAddress, ERC20_ABI, "balanceOf", [address], network),
      ]);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const response = await writeContract(mneeAddress, ERC20_ABI, "approve", [spenderAddress, approveAmount]);
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <Coins className="h-4 w-4" />
            MNEE Token ({network === "mainnet" ? "Mainnet" : "Sepolia"})
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="text-sm">
            ...
            <a
              href={getBlockExplorerUrl(network, mneeAddress)}
              target="_blank"
              rel="noopener noreferrer"
              className="font-mono text-xs hover:underline inline-flex items-center gap-1"
            >
              {mneeAddress.slice(0, 10)}...{mneeAddress.slice(-8)}
              <ExternalLink className="h-3 w-3" />
            </a>
          </div>
          <div className="text-sm">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Approve MNEE Spending</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Before using functions like stake() or pay(), you need to approve this contract to spend your MNEE tokens.
          </p>
          <div>
            <label className="text-xs text-muted-foreground">Amount (raw units, MNEE has 2 decimals)</label>
            <div className="flex gap-2">
              <Input
                placeholder="e.g., 10000000000 for 100M MNEE"
                value={approveAmount}
          ...
          {txHash && (
            <div className="p-2 bg-green-50 dark:bg-green-950 rounded text-sm">
              ...
              <a
                href={`https://${network === "sepolia" ? "sepolia." : ""}etherscan.io/tx/${txHash}`}
                target="_blank"
                rel="noopener noreferrer"
...
function FunctionCard({ fn, contractAddress, network, abi, isWrite }: FunctionCardProps) {
  ...
  const handleQuery = async () => {
    ...
    try {
      ...
      if (message.includes("timeout") || message.includes("abort")) {
        setError("Request timed out. Try switching MetaMask to the correct network.");
      } else {
        setError(message);
      }
    } finally {
  ...
  const handleWrite = async () => {
    ...
    try {
      ...
      if (currentNetwork !== network) {
        const switched = await switchNetwork(network);
        if (!switched) {
          throw new Error(`Please switch to ${network === "mainnet" ? "Ethereum Mainnet" : "Sepolia"}`);
        }
      }
